...
const UserPage = ({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await getQuestionsByFilter(
          "newest",
          `.${userDetails.name}.`
        );
        setQlist(res || []);
  ...
  useEffect(() => {
    const fetchAnswers = async () => {
      try {
        const response = await getAllAnswersByUid(userDetails._id);
    ...
    <div style={profilePageStyle}>
      <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
        ...
        >
          ...
          <div style={impressionBoxStyle}>
            ...
            <Typography variant="h5">{Math.floor(userDetails.impressions/2)}</Typography>
          </div>
        </div>
        <div>
          ...
          {qlist.length === 0 ? (
            <Typography variant="body1" style={noContentStyle}>
              No questions posted by the user
            </Typography>
          ) : (
      ...
      {/* {page === "editProfile" && (
        <EditProfile
          ...
          setPage={handleRedirectToProfile}
